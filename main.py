...
@with_db
def get_subscription_id_from_token(db: sqlite3.Connection, token: str) -> Optional[int]:
...
@with_db
def get_or_create_session(db: sqlite3.Connection, session_id: Optional[str], now: float, subscription_id: int) -> Tuple[str, List[Dict[str, Any]]]:
    """Get existing session or create a new one, returning session_id and messages."""
    # If session_id is provided and exists, use it
    if session_id:
        logging.info(f"Looking for existing session: {session_id}")
        cur = db.execute(
        ...
        if row and now - row['last_access'] < SESSION_TIMEOUT_SECONDS:
            ...
            db.commit()
            logging.info(f"Session {session_id} expired, deleted")
    ...
    if row and now - row['last_access'] < SESSION_TIMEOUT_SECONDS:
        ...
        db.commit()
        logging.info(f"Resuming most recent session: {row['id']}")
        return row['id'], json.loads(row['messages'])
    
    # Create a new session
    new_session_id = str(uuid.uuid4())
    logging.info(f"Creating new session: {new_session_id}")
    messages = [
        {"role": "system", "content": "You are a helpful piggy bank assistant. You have access to a set of tools to manage accounts. When a user asks for multiple pieces of information, you can call multiple tools in parallel."}
    ]
...
@with_db
def update_session_messages(db: sqlite3.Connection, session_id: str, messages: List[Dict[str, Any]]) -> None:
...
def process_openai_response(messages: List[Dict[str, Any]]) -> Any:
    ...
    logging.debug(f"Received response: {response_message.content}")
    
    if tool_calls:
        logging.info(f"Tool calls requested: {[f'{tc.function.name}({tc.function.arguments})' for tc in tool_calls]}")
...
def agent() -> Union[Response, Tuple[Response, int]]:
    ...
    try:
        ...
        logging.error(f"Error in OpenAI integration: {e}")
